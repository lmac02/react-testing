...
export class FilterControl extends React.Component {
    render() {
        const { searchValue, appliedFilters = [], additionalAction, focused = false, filters = [], onSearchBlur, onSearchChange, polaris: { intl }, context: { selectMode, resourceName }, } = this.props;
        ...
        const filterCreatorMarkup = filters.length > 0 ? (<FilterCreator resourceName={resourceName} filters={filters} onAddFilter={this.handleAddFilter} disabled={selectMode}/>) : null;
        const appliedFiltersMarkup = appliedFilters.map((appliedFilter) => {
          ...
          <Tag onRemove={this.getRemoveFilterCallback(filterId)} disabled={selectMode}>
            {activeFilterLabel}
          </Tag>
        </li>);
        });
        ...
        return (<FormLayout>
        <TextField connectedLeft={filterCreatorMarkup} connectedRight={additionalActionButton} label={textFieldLabel} labelHidden placeholder={textFieldLabel} prefix={<Icon source="search" color="skyDark"/>} value={searchValue} onChange={onSearchChange} onBlur={onSearchBlur} focused={focused} disabled={selectMode}/>
    ...
    getRemoveFilterCallback(filterId) {
        return () => {
...
tslib_1.__decorate([
    autobind
], FilterControl.prototype, "handleAddFilter", null);
tslib_1.__decorate([
    memoize()
], FilterControl.prototype, "getRemoveFilterCallback", null);
...
export default compose(withAppProvider(), withContext(Consumer))(FilterControl);
